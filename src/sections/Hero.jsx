...
const Hero = () => {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center bg-gradient-animated relative overflow-hidden">
      ...
      <div className={`absolute rounded-full mix-blend-multiply filter bg-portfolio-3 pointer-events-none ${isDesktop ? 'bottom-20 left-1/4 w-[400px] h-[400px] blur-3xl opacity-20 pulse-intense' : 'top-[45%] -left-10 w-32 h-32 blur-xl opacity-12'}`} style={isDesktop ? { animationDelay: '2s' } : {}}></div>
      <div className={`absolute rounded-full mix-blend-multiply filter bg-gradient-to-br from-portfolio-1 to-portfolio-2 pointer-events-none ${isDesktop ? '-bottom-20 -right-20 w-[450px] h-[450px] blur-3xl opacity-25 pulse-intense' : 'top-[20%] -right-10 w-36 h-36 blur-xl opacity-10'}`} style={isDesktop ? { animationDelay: '1.5s' } : {}}></div>
